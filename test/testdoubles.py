...
class HDL32ePcapDouble:
    ...
    def get_packet_header(self, port=2368):
        """
        Copied from a valid PCAP file
        bytes 36-38
    ...
    def build_firing_from_numpy(self, azi, dist_row, int_row):
        """
    ...
    def get_single_return_packet(self, azi, dist, intens,
            port=2368, timestamp=0):
        header = self.get_packet_header(port)
        payload = self.numpy_data_to_packet_payload(azi, dist, intens) 
        ts = timestamp.to_bytes(4, 'little')
        factory = np.random.bytes(2)
        return header +payload + ts + factory

    def numpy_data_to_packet_payload(self, azi, dist, intens):        
        ...
        payload = b''
        for i in range(0, 12):
            payload += self.build_firing_from_numpy(
                azi_pcap[i, 0],
                dist_pcap[i, :],
                intens_pcap[i, :]
            )
